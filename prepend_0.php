...
function rewrite_vhosts()
{
	...
	foreach ( $httpd_conf_array as $httpd_line ) {
		...
		if ( $get_server_name ) {
			if ( FALSE !== strpos( $httpd_line, 'erverNam' ) ) {
		...
		if ( FALSE !== strpos($httpd_line,'SLCertificat') ) {
			$httpd_line = str_replace( 'server', $domain, $httpd_line );
		}

		//reset for closed virtualhost
		if ( FALSE !== strpos( $httpd_line, '/VirtualHost' ) ) {
...
function create_ssl($domain = null, $keypath, $certpath)
{
	...
	if ( '' !== $siteName ) {
		...
		$ssl_temp_file = fopen( local_ssl_path . '_' . $siteName . '_v3.ext', 'w' )
			or die( 'Unable to open file: ' . local_ssl_path . '_' . $siteName . '_v3.ext' . ' !' );
